...
class MemoryEntry:
    ...
    def __init__(
        self, content: str, memory_type: str = "episodic", metadata: dict | None = None, importance: float = 0.5
    ):
        """
...
class AgentMemory:
    ...
    def __init__(
        ...
        if CHROMADB_AVAILABLE:
            self.client = chromadb.Client(
                Settings(chroma_db_impl="duckdb+parquet", persist_directory=str(self.persist_directory))
            )
    ...
    async def store(
        self, content: str, memory_type: str = "episodic", importance: float = 0.5, metadata: dict | None = None
    ) -> str:
        """
        Store a new memory.
        ...
        if self.collection:
            self.collection.add(
                embeddings=[embedding], documents=[content], metadatas=[entry.to_dict()], ids=[entry.id]
            )
        else:
            # Fallback storage
            self._fallback_memory.append(
                {"id": entry.id, "content": content, "embedding": embedding, "metadata": entry.to_dict()}
            )
    ...
    async def retrieve(
        self, query: str, memory_type: str | None = None, top_k: int = 5, importance_threshold: float = 0.0
    ) -> list[dict[str, Any]]:
        """
        ...
        if self.collection:
            ...
            for mem in self._fallback_memory:
                sim = cosine_similarity([query_embedding], [mem["embedding"]])[0][0]
    ...
    async def retrieve_recent(self, n: int = 10) -> list[dict[str, Any]]:
        ...
        return [{"content": mem.content, "metadata": mem.to_dict(), "id": mem.id} for mem in reversed(recent)]
...
class WorkingMemory:
    ...
    def add(self, content: str, role: str = "user", metadata: dict | None = None):
        ...
        Args:
            ...
            metadata: Additional metadata
        """
        entry = {"content": content, "role": role, "timestamp": datetime.now().isoformat(), "metadata": metadata or {}}
