...
class SummaryComparator:
    ...
    def compare_summaries(
        self, summaries: list[str], original_text: str = None, threshold: float = 0.75
    ) -> dict[str, Any]:
        """
        ...
        if original_text:
            content_preservation = self._analyze_content_preservation(summaries, original_text)

        # Find consensus summary
        consensus_idx = self._find_consensus_summary(similarity_matrix)

        # Identify significant differences
        differences = self._identify_differences(summaries, similarity_matrix, threshold)
    ...
    def _calculate_coherence(self, summaries: list[str]) -> list[float]:
        ...
        for summary in summaries:
            ...
            similarities = util.pytorch_cos_sim(sentence_embeddings[:-1], sentence_embeddings[1:])
    ...
    def _analyze_content_preservation(self, summaries: list[str], original_text: str) -> list[float]:
        """
    ...
    def _identify_differences(self, summaries: list[str], similarity_matrix, threshold: float) -> list[dict[str, Any]]:
        """
